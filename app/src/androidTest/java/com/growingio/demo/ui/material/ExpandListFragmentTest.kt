...
@LargeTest
class ExpandListFragmentTest : AbstractGrowingTestUnit() {

    @Before
    override fun setup() {
        super.setup()
        launchFragmentInHiltContainer<ExpandableListViewFragment>()
    }

    @Test
    fun onExpandGroupClickTest() {
        runEventTest(AutotrackEventType.VIEW_CLICK, onEvent = { baseEvent ->
            ...
            onData(hasToString("我的家人"))
                .inAdapterView(withId(R.id.expandableListView))
                .perform(click())
        }
    }

    fun onExpandChildClickTest() {
    }
}
