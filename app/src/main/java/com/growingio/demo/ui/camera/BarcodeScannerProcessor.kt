...
class BarcodeScannerProcessor(context: Context, private val barcodeCallback: (codes: List<String>) -> Unit?) :
    ...
    override fun onSuccess(barcodes: List<Barcode>, graphicOverlay: GraphicOverlay) {
        ...
        for (i in barcodes.indices) {
            ...
            if (barcode.rawValue != null) list.add(barcode.rawValue!!)
            logExtrasForTesting(barcode)
        }
        if (list.isNotEmpty()) {
            barcodeCallback.invoke(list)
        }
    }
